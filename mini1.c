...
void semantic_check(AST *now) {
	...
	if(now->kind == PREINC || now->kind == PREDEC || now->kind == POSTINC || now->kind == POSTDEC){
		AST *tmp = now->mid;
		while (tmp->kind == LPAR) tmp = tmp->mid;
		if (tmp->kind != IDENTIFIER) err("Operand of INC/DEC must be an identifier or identifier with parentheses.");
	}
...
int is_constant(AST *root){ //check is constant or not
    while(root->kind == LPAR){
        if(root->mid->kind != LPAR && root->mid->kind != CONSTANT) return 0; //is other kind
        if(root->mid->kind == LPAR) root = root->mid; //continue find
        else if(root->mid->kind == CONSTANT) return 1; //have constant
        else return 0;
    }
}
...
int have_identifier(AST *root){  //check have identifier or not
    while(root->kind == LPAR){
        if(root->mid->kind != LPAR && root->mid->kind != IDENTIFIER) return 0; //is other kind
        if(root->mid->kind == LPAR) root = root->mid; //continue find
        else if(root->mid->kind == IDENTIFIER) return root->mid->val; //return x/y/z
        else return 0;
    }
	return 0;
}
...
int codegen(AST *root) {
    ...
    switch (root->kind) {
        case ASSIGN:
            ...
            if(vr == 0) vr = root->lhs->val;
			rv = codegen(root->rhs);
            is_rc = 0;
            if(root->rhs->kind == CONSTANT) is_rc = 1;
            if(root->rhs->kind == LPAR && is_constant(root->rhs)) is_rc = 1;
            if(is_rc == 1) {
        ...
        case ADD:
            ...
            is_lc = 0;
            is_rc = 0;
            if(root->lhs->kind == CONSTANT) is_lc = 1;
            if(root->rhs->kind == CONSTANT) is_rc = 1;
            if(root->lhs->kind == LPAR && is_constant(root->lhs)) is_lc = 1;
            if(root->rhs->kind == LPAR && is_constant(root->rhs)) is_rc = 1;
            if((is_lc == 0) & (is_rc == 0)) {
        ...
        case MINUS:
            lv = codegen(root->mid);
            is_lc = 0;
            if(root->mid->kind == CONSTANT) is_lc = 1;
            if(root->mid->kind == LPAR && is_constant(root->mid)) is_lc = 1;
            if((is_lc == 1)) {
                for(int i=0; i<NUM_REGISTERS; i++){
                    if(reg[i] == 0){
                        reg[i] = 1;
